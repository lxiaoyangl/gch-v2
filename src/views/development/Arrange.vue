<template>
  <div class="arrange">
    <div class="cont">
      <div class="left">
        <div class="comp-class">
          ...
          >
            <template v-for="(item, i) in isFlow ? flowList : compClassList">
              <div class="item" @click="handleChange(i)" :class="active == i && 'active'">
        ...
        <div class="flow">
          ...
          <div class="list" v-if="compList.length > 0">
            <div class="item" :class="activeName == '流程组件' ? 'flo' : 'other'" v-for="(item, i) in compList">
              <!-- <el-tooltip
                effect="light"
                :content="item.desc"
              ...
              <i @click="handleDelComp(i)" class="CircleClose"></i>
              <!-- </el-tooltip> -->
            </div>
          </div>
      ...
      <div class="right">
        <el-form v-if="isFlow" inline class="flow-top" :model="flowTopForm" label-width="100px">
    ...
    <div class="foot">
      ...
      <el-button type="success" @click="commplete">保存草稿</el-button>
      <el-button type="info" @click="goBack">上一步</el-button>
      <el-button type="primary" @click="reset">下一步</el-button>
    </div>
...
import { Graph, Shape, Addon, NodeView, View, Cell } from "@antv/x6";
...
let isFlow: Ref<boolean> = ref(false);
let nodeDrawer: Ref<boolean> = ref(false);
let lineDrawer: Ref<boolean> = ref(false);
let flowDrawer: Ref<boolean> = ref(false);
let isDebug: Ref<boolean> = ref(false);
...
const compList = computed(() => {
  ...
  if (isFlow.value) {
    arr = arr.filter((item: ListDataChildren) => item.name !== "流程组件");
  }
  return arr.find((v: any, i: number) => i == active.value).children || [];
});
...
const activeName = computed(() => {
  ...
  return arr.find((v: any, i: number) => i == active.value).name;
});

const initAntvX6 = () => {
  const containerChart = document.getElementById("containerChart") as HTMLDivElement;
  graph = new Graph({
    // width: 100,
    // height: 100,
    ...
    connecting: {
      ...
      createEdge() {
        return new Shape.Edge({
          ...
          attrs: {
            line: {
              stroke: "#4E4E4E", // stroke: "#45D9BF", stroke: "#FD9F6C",
              strokeWidth: 2,
              // targetMarker: {
              //   name: "classic",
              //   size: 12,
  ...
  graph.on("node:mouseenter", ({ e, x, y, node, view }) => {
    ...
    // 添加连接点;
    const ports = containerChart.querySelectorAll(".x6-port-body");
 ...
 * 拖拽节点，并添加到画布中
 */
const dragNode = (item: ListDataChildren, e: Event) => {
  lineDrawer.value = !1;
 ...
 * 创建矩形节点
 */
const createRectNode = (item: ListDataChildren) => {
  // 定义节点
  ...
  // Shape.Rect.prototype.updateInPorts = function (graph: Graph) {
  //   const minNumberOfPorts = 3;
...
// 组件关闭按钮
const handleDelComp = (index: number) => {
